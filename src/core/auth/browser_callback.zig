...
fn listenerReady(
    ...
    // Zig 0.16's Windows Io backend owns AFD handles rather than Winsock
    // sockets, so std.posix.pollfd is intentionally unavailable. Accept is
    // still cancelable through the Io backend; let it wait for the callback.
