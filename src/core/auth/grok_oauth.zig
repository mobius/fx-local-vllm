...
const StdinManualCodeReader = struct {
    ...
    fn poll(self: *StdinManualCodeReader) !?[]const u8 {
        ...
        // Zig 0.16 does not expose POSIX pollfd on Windows. The browser
        // callback remains the primary Windows login path; manual-code input
        // is retained on POSIX targets where stdin polling is available.
