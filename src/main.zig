...
test {
    if (comptime builtin.os.tag == .windows) {
        // This registry intentionally exercises Unix-only integration fixtures
        // (fork, pollfd, PTY, Unix signals, and POSIX descriptor semantics).
        // Windows uses the focused test runner below instead.
        return;
    }
